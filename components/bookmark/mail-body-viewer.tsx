...
const MailBodyViewer: React.FC<MailBodyViewerProps> = ({ htmlContent, mailId }) => {
  ...
  useEffect(() => {
    ...
    iframe.onload = () => {
      ...
      const sortedBookmarks = [...currentMailBookmarks].sort((a, b) => {
        ...
        const rangeA = deserializeRange(a.serializedRange, doc.body);
        const rangeB = deserializeRange(b.serializedRange, doc.body);
        if (!rangeA || !rangeB) return 0;
        return rangeB.compareBoundaryPoints(Range.START_TO_START, rangeA);
      });

      sortedBookmarks.forEach(bookmark => {
        if (!doc.body || !bookmark.serializedRange) return;
        const range = deserializeRange(bookmark.serializedRange, doc.body);
      ...
      sortedBookmarks.forEach(bookmark => {
        ...
        const highlightSpans = Array.from(
          doc.body?.querySelectorAll(`.bookmark-highlight[data-bookmark-id="${bookmark.id}"]`) || []
        ) as HTMLElement[];
  ...
  const handleMouseUp = useCallback((event: MouseEvent) => {
    ...
    setTimeout(() => {
      ...
      if (text.length > 0 && iframe.contentWindow?.document.body.contains(range.commonAncestorContainer)) {
        const rect = range.getBoundingClientRect();
        
        const newBookmark = addBookmark(text, range, iframe.contentWindow.document.body, mailId); 
        if (newBookmark) {
  ...
  useEffect(() => {
    ...
    if (doc) {
      ...
      return () => doc.removeEventListener('mouseup', handleMouseUp as EventListener);
    }
  }, [handleMouseUp]);
