...
export const OnboardingFlow = () => {
  ...
  const [modalState, setModalState] = useState<"loading" | "open" | "closed">(
    "loading"
  );
  const [activeSubFlow, setActiveSubFlow] = useState<SubFlow>("none");
  const [serviceForSelection, setServiceForSelection] = useState<
    "Gmail" | "Outlook" | null
  >(null);
  ...
  // Add a ref to track if we've already checked the status
  const hasCheckedStatus = useRef(false);

  useEffect(() => {
    const checkStatus = async () => {
      // Only check status once per component lifecycle
      ...
      try {
        ...
        if (!isComplete) {
          ...
          if (
            urlParams.has("gmail_connected") ||
            urlParams.has("outlook_connected")
          ) {
            goToStep(3);
          }
        }
    ...
    case "gmail_connect":
      return (
        <GmailConnectionFlow
          ...
          onConnectionComplete={handleSubFlowClose}
        />
      );
          ...
          case 3:
            return (
              <ConnectStep
                ...
                onSelectSenders={handleStartSelectSenders}
              />
            );
          case 4:
            return <GetAppStep />;
