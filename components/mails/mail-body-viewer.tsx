...
function sanitizeMailHtml(html: string): string {
  ...
  // Optionally, remove <script> tags for safety
  html = html.replace(/<script[\s\S]*?>[\s\S]*?<\/script>/gi, '');
  // Optionally, remove <base> tags which can affect relative URLs
...
const MailBodyViewer: React.FC<MailBodyViewerProps> = ({ htmlContent, mailId }) => {
  ...
  useEffect(() => {
    ...
    // 1. Always reset to sanitized HTML before applying highlights
    const safeHtml = sanitizeMailHtml(htmlContent);
    contentRef.current.innerHTML = safeHtml;

    // 2. Apply highlights only to the mail body (not title)
    //    (Assumes contentRef only contains mail body)

    // 3. Sort bookmarks for correct highlight order
    ...
    // 4. Apply highlights for each bookmark
    sortedBookmarks.forEach(bookmark => {
      ...
      // If range is invalid, skip this bookmark
    ...
    // 5. Add indicator icons and event handlers
