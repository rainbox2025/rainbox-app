...
const highlightRange = (
  ...
  nodes.forEach((node) => {
    ...
    try {
      ...
      console.warn("Could not highlight a segment:", e);
    }
  });
};
...
const MailBodyViewer: React.FC<MailBodyViewerProps> = ({
  ...
  useEffect(() => {
    ...
    if (!rootElement || !htmlContent) return;

    rootElement.innerHTML = sanitizeMailHtml(htmlContent);

    // FIX: Sort bookmarks in reverse document order before applying.
    // This prevents highlights at the start of a node from invalidating the ranges
    // of highlights that appear later in the same node.
    const sortedBookmarks = [...currentMailBookmarks].sort((a, b) => {
      if (!a.serializedRange || !b.serializedRange) return 0;
      // A simple path and offset comparison for reverse sorting.
      const pathA = a.serializedRange.start.path;
      const pathB = b.serializedRange.start.path;
    ...
    sortedBookmarks.forEach((bookmark) => {
      ...
      if (range && rootElement.contains(range.commonAncestorContainer)) {
        highlightRange(range, bookmark.id, bookmark.isConfirmed !== false);
      }
    });
